...
class AuthCubit extends Cubit<AuthStates> {
  ...
  signUp(userModel) {
    var response =
    Api().postHttp(url: 'register', data: userModel.toJson());
    emit(SignUpLoading());
    print(response);
    response.then((value) =>
    {
      print(value),
      emit(SignUpSuccess()),
      showToast(msg: 'Sign up successfully', state: ToastedStates.SUCCESS),
  ...
  notLogged(email) {
    var response = Api().postHttp(
        url: 'recover-password', data: {"username": "$email"});
emit(RecoverPasswordLoading());
    response.then((value) =>
    {
      ...
      print(response),
      print('Success'),
    showToast(msg: 'Success!A password reset link has been sent', state:ToastedStates.SUCCESS ),
