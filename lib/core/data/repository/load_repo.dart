...
class LoadsRepo{
  static Future<List<Vehicles>> getLoads(self,
      {val,  equipmentSize, attributes,
    vehicleSize, vehicleType, context,isFilter
      }
      )async {
    ...
    print(self);
    var response;
    response = await Api().getHttp(url: 'loads',authToken: token,self: self,data:{"search":val});
    print(response);
    print('loadrepoo');


    isFilter?
    response = await Api().getHttp(url: 'loads',authToken: token,self: self,
        data: {   "equipment_types": equipmentSize,
  ...
  static Future<List<GetLoadsModel>> getLoads1(self)async {
    String token=await CacheHelper.getString(SharedKeys.token);
    print(token);
    print(self);
    var response;
